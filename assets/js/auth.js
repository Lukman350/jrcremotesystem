...
function login() {
	$.ajax({
		...
		success: function (result) {
			const { status, inputerror, texterror } = result;
			// console.log(status);
			...
			if (result.status) {
				...
				if (result.inputerror.length > 0) {
					for (var i = 0; i < result.inputerror.length; i++) {
						...
						$('[name="' + result.inputerror[i] + '"]')
							.next()
							.text(result.texterror[i]);
					}
				}
			}
		},
		...
		success: function (result) {
			if (result.status) {
				...
				if (result.inputerror) {
					for (var i = 0; i < result.inputerror.length; i++) {
						$('[for ="' + result.inputerror[i] + '" ]').addClass("text-danger");
						$('[name ="' + result.inputerror[i] + '" ]').removeClass(
							"border-primary"
						);
						$('[name ="' + result.inputerror[i] + '" ]').addClass(
							"border-danger"
						);
						$('[name="' + result.inputerror[i] + '"]')
							.next()
